...
public class CreditCodeUtil {
	...
	public static String randomCreditCode() {
		...
		//
		for (int i = 0; i < 2; i++) {
			int num = com.daluobai.jenkinslib.lib.hutool.core.util.RandomUtil.randomInt(BASE_CODE_ARRAY.length);
			buf.append(Character.toUpperCase(BASE_CODE_ARRAY[num]));
		}
		for (int i = 2; i < 8; i++) {
			int num = com.daluobai.jenkinslib.lib.hutool.core.util.RandomUtil.randomInt(10);
