package com.daluobai.jenkinslib.lib.hutool.core.io.file;
...
import com.daluobai.jenkinslib.lib.hutool.core.io.file.visitor.MoveVisitor;
...
public class PathMover {
	...
	public PathMover(final Path src, final Path target, final CopyOption[] options) {
		...
		if(false == com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.exists(src, false)){
	...
	public Path move() {
		...
		if (com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.isSub(src, target)) {
			if(Files.exists(target) && com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.equals(src, target)){
		...
		if (com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.isDirectory(target)) {
			// 创建子路径的情况，1是目标是目录，需要移动到目录下，2是目标不能存在，自动创建目录
			target = target.resolve(src.getFileName());
		}

		// 自动创建目标的父目录
		com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.mkParentDirs(target);
		try {
			...
			// 移动后删除空目录
			com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.del(src);
			return target;
		}
	}
	...
	public Path moveContent() {
		...
		if (com.daluobai.jenkinslib.lib.hutool.core.io.file.PathUtil.isExistsAndNotDirectory(target, false)) {
			// 文件移动调用move方法
			return move();
		}
